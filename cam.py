                ...
                if current_frame is not None and id(current_frame) != last_processed_frame_id:
                    try:
                        ...
                        if self.frame_skip_counter >= self.process_every_nth_frame:
                            ...
                            # Run inference
                            result = self.model.predict(small, confidence=45, overlap=25)
                            ...
                            # Extract detection data correctly from the Detections object
                            if hasattr(result, 'pandas') and callable(getattr(result, 'pandas')):
                                # For YOLOv5 detections
                                detections_df = result.pandas().xyxy[0]
                                for _, det in detections_df.iterrows():
                                    x1 = int(det['xmin'] * scale_x)
                                    y1 = int(det['ymin'] * scale_y)
                                    x2 = int(det['xmax'] * scale_x)
                                    y2 = int(det['ymax'] * scale_y)
                                    conf = float(det['confidence'])
                                    cls_name = det['name'] if 'name' in det else f"Class {det['class']}"
                                    
                                    cv2.rectangle(processed, (x1, y1), (x2, y2), (0, 255, 0), 2)
                                    cv2.putText(processed, f"{cls_name} {conf:.0%}", 
                                              (x1, y1-5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
                            elif hasattr(result, 'pred') and len(result.pred) > 0:
                                # Alternative way to access YOLOv5 detections
                                for *xyxy, conf, cls in result.pred[0]:
                                    ...
                                    cv2.rectangle(processed, (x1, y1), (x2, y2), (0, 255, 0), 2)
                                    cv2.putText(processed, f"{conf:.0%}", 
                                              (x1, y1-5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
                            else:
                                # Fallback for other detection formats
                                cv2.putText(processed, "Detection format not supported", 
...
def main():
    ...
    stream = RealTimeESP32Stream("http://172.20.10.2/stream", model)
